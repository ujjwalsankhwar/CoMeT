...
def usage():
  ...
  print ('''Detailed options: \n
     ...
     --which_DRAM: Layer of which DRAM you want to view (default 0)
...
opts_passthrough = [ 'cores_in_x=', 'cores_in_y=', 'cores_in_z=', 'banks_in_x=', 'banks_in_y=', 'banks_in_z=', 'arch_type=', 'which_DRAM=', 'plot_type=', 'layer_to_view=', 'type_to_view=', 'verbose', 'inverted_view', 'debug', 'tmin=', 'tmax=', 'samplingRate=', 'traceFile=', 'output=', 'clean' ]
...
temperatures_bank = [0] * total_banks*num_DRAM
temperatures_DRAMS = []
...
def parse_tfile_header(tfilename, orig_line):
    ...
    if (count_bank_column!=total_banks*num_DRAM):
        print("WARNING: Total bank columns does not match the number of banks!!")
        print("    Number banks:%d, number columns:%d" %(total_banks*num_DRAM, count_bank_column))
...
if __name__ == "__main__":
    ...
    for line in lines[1:]:
        ...
        if (count%samplingRate != 0):
            count+=1
            continue
        fig = plt.figure(figsize=(32,16))#(14, 8))
        ...
        if (plot_type == "2D"):
            ...
            gs = gridspec.GridSpec(nrows=1, ncols=6, width_ratios=[1, 1, 1, 1, 1, 0.5])# 1, 1, 1]) 
...
#            art3d.pathpatch_2d_to_3d(p, z=0, zdir = "x")
            ...
            if (arch_type=="3D"):
                ...
                postprocess=True
                nth_DRAM = 0
                #print(len(temperatures_bank))
                #print(temperatures_bank)
                while (nth_DRAM<num_DRAM):
                    if (arch_type == "2.5D"):
                        title_message = "Mem."+str(nth_DRAM)+" temperature map"
                    else:
                    ...
                    xwidth=1
                    ywidth=1
                    ax = fig.add_subplot(gs[1+nth_DRAM], projection='3d')
                    #nth_temperatures_bank = temperatures_bank[nth_DRAM*banks_in_x*banks_in_y*banks_in_z:(nth_DRAM+1)*banks_in_x*banks_in_y*banks_in_z]
                    nth_bank_temperature = []
                    z1 = 0
                    while (z1<banks_in_z):
                        nth_bank_temperature = nth_bank_temperature + temperatures_bank[(nth_DRAM*banks_in_x*banks_in_y)+z1*(banks_in_x*banks_in_y*num_DRAM):(nth_DRAM*banks_in_x*banks_in_y)+z1*(banks_in_x*banks_in_y*num_DRAM)+banks_in_x*banks_in_y]
                        z1+=1
                    #print(nth_bank_temperature)
                    #if (nth_DRAM==which_DRAM and ==layer_to_view):
                    temperatures_DRAMS.append(nth_bank_temperature)
                    #print(len(temperatures_DRAMS))
                    #print(nth_bank_temperature)
                    plot_3D_structure(ax, tmin, tmax, banks_in_x, banks_in_y, banks_in_z, zstart, xwidth, ywidth, temperatures_bank, title_message, postprocess, inverted_view, "Mem."+str(nth_DRAM), adj)
                    nth_DRAM+=1
            #ax_h, ax_w = 4, 4 ##
            ...
            plt.annotate(arch_string, xy=(0.13, 0.95), xycoords='figure fraction', fontsize=21)
            plt.annotate("Time step = "+str(count) +" ms", xy=(0.13, 0.87), xycoords='figure fraction', fontsize=21)
            if (inverted_view):
                ...
                plt.annotate("Heat Sink at top", xy=(0.13, 0.79), xycoords='figure fraction', fontsize=21)
...
##            ##Adding the bottom layer view
            ...
            if (type_to_view == "CORE"):
                ...
                #which_mem = 0
                start_index = layer_to_view * xdim * ydim
                end_index = start_index + banks_in_x*banks_in_y
                temperatures = temperatures_DRAMS[which_DRAM][start_index : end_index]
                title_message = "MEM."+str(which_DRAM)+" layer " + str(layer_to_view)
            else:
...
##
##
# :~/CoMeT/test/SPEC_Exp/WorkArea/4core_lbm_2_5D
# $ python3 ../../../../scripts/heatViewMultiDRAM.py --cores_in_x 2 --cores_in_y 2 --cores_in_z 2 --banks_in_x 4 --banks_in_y 4 --banks_in_z 8 --arch_type 2.5D --which_DRAM 0 --layer_to_view 0 --traceFile combined_temperature_4DRAM.trace --output maps_4DRAM
